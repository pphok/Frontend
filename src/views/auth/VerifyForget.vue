...
const submitForm = async () => {
    if (!/^\d{6}$/.test(otp.value)) {
        ...
        await store.VerifyForget(formData.value);
        if (store.error) {
        ...
        router.replace({
            path: route.query.to ? String(route.query.to) : '/change-password',
            query: {
...
onMounted(() => {
    const queryParamValue = route.query.email;
    if (queryParamValue) {
        formData.value.email = queryParamValue;
    }
});
</script>
